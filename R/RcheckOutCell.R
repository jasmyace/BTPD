RcheckOutCell <- function(userID,tblDir="//lar-file-srv/Data/BTPD_2016/Digitizing",shpDir="//lar-file-srv/Data/BTPD_2016/Analysis/data/Shapefiles/BTPD_Grid_CO_Ranked",shp="BTPD_Grid_CO_Ranked"){
  ...
  if( sum(open) == 0 & sum(digi) == 0 & sum(buff) == 0 & sum(done) == nTotal ){
    ...
    #   ---- list, we have to check and see if it's neighbors are available. 
